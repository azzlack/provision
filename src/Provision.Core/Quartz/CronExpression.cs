...
#region Purpose
/*
 * A portable version of the <c>CronExpression</c> class from Quartz.NET
 */
#endregion

namespace Provision.Core.Quartz
{
    using System;
    ...
    using Provision.Core.Quartz.Collection;
    ...
    public class CronExpression
    {
        ...
        public CronExpression(string cronExpression)
        {
            ...
            this.cronExpressionString = cronExpression.ToUpper();
        ...
        public virtual bool IsSatisfiedBy(DateTimeOffset dateUtc)
        {
            ...
            DateTimeOffset? timeAfter = this.GetTimeAfter(test);
        ...
        public virtual DateTimeOffset? GetNextInvalidTimeAfter(DateTimeOffset date)
        {
            ...
            {
                DateTimeOffset? newDate = this.GetTimeAfter(lastDate);
        ...
        protected void BuildExpression(string expression)
        {
            ...
            try
            {
                ...
                ISortedSet<int> dow = this.GetSet(DayOfWeek);
                ISortedSet<int> dom = this.GetSet(DayOfMonth);
        ...
        protected virtual int StoreExpressionVals(int pos, string s, int type)
        {
                ...
                else if (type == DayOfWeek)
                {
                    ...
                    if (s.Length > i + 3)
                    {
                        ...
                        else if (c == '#')
                        {
                            ...
                            {
                                i += 4;
                                this.nthdayOfWeek = Convert.ToInt32(s.Substring(i), CultureInfo.InvariantCulture);
            ...
            if (c == '?')
            {
                ...
                if (type == DayOfWeek && !this.lastdayOfMonth)
                {
                    int val = this.daysOfMonth[this.daysOfMonth.Count - 1];
                    if (val == NoSpecInt)
                    {
                        throw new FormatException(
                            "'?' can only be specified for Day-of-Month -OR- Day-of-Week.");
            ...
            else if (c == 'L')
            {
                ...
                if (type == DayOfMonth && s.Length > i)
                {
                    ...
                    if (c == '-')
                    {
                        ...
                        this.lastdayOffset = vs.theValue;
        ...
        protected virtual int CheckNext(int pos, string s, int val, int type)
        {
            ...
            if (c == 'L')
            {
                ...
                ISortedSet<int> data = this.GetSet(type);
                data.Add(val);
                i++;
                return i;
            }
        ...
        {
            get { return this.cronExpressionString; }
        }
        ...
        public virtual DateTimeOffset? GetTimeAfter(DateTimeOffset afterTimeUtc)
        {
            ...
            d = TimeZoneInfo.ConvertTime(d, this.TimeZone);
            ...
            while (!gotOne)
            {
                ...
                // get day...................................................
                bool dayOfMSpec = !this.daysOfMonth.Contains(NoSpec);
                bool dayOfWSpec = !this.daysOfWeek.Contains(NoSpec);
                if (dayOfMSpec && !dayOfWSpec)
                {
                    ...
                    if (this.lastdayOfMonth)
                    {
                        ...
                        {
                            t = day;
                            day = this.GetLastDayOfMonth(mon, d.Year);
                    ...
                    else if (st != null && st.Count != 0)
                    {
                        ...
                        int lastDay = this.GetLastDayOfMonth(mon, d.Year);
                        if (day > lastDay)
                        {
                            day = this.daysOfMonth.First();
                            mon++;
                ...
                else if (dayOfWSpec && !dayOfMSpec)
                {
                    ...
                    if (this.lastdayOfWeek)
                    {
                        ...
                        // the month?
                        int dow = this.daysOfWeek.First(); // desired
                        // d-o-w
        ...
        public object Clone()
        {
            ...
            try
            {
                copy = new CronExpression(this.CronExpressionString);
